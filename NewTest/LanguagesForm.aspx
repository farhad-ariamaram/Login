<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LanguagesForm.aspx.cs" Inherits="NewTest.LanguagesForm" %>
    ...
    <script>
        ...
        function deleteitem(idd) {
            ...
            if (r == true) {
                ...
                $.ajax({
                    type: "POST",
                    url: "WebServices.aspx/deleteLanguage",
                    contentType: "application/json;charset=utf-8",
                    dataType: "json",
        ...
        function updateitem(idd) {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices.aspx/readLanguageById",
                contentType: "application/json;charset=utf-8",
                dataType: "json",
                data: JSON.stringify(data),
                success: function (response) {
                    $.each(response.d, function (i, item) {
                        $('#txtText').val(item._Fld_Sitestyle_Language_Text);
                        $('#DropdownDir').val(item._Fld_Sitestyle_Language_Direction);
                        $("#DropdownIso1").val(item._Fld_Sitestyle_Language_Iso639_1_Id);
                        $("#DropdownIso2").val(item._Fld_Sitestyle_Language_Iso639_2_Id);
                        $("#DropdownIso3").val(item._Fld_Sitestyle_Language_Iso639_3_Id);
                    });
                },
                failure: function () {
                    alert("there is a error!");
        ...
        function refreshdata() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    $.each(response.d, function (i, item) {
                        $('<tr>').html("<td>" + item._Fld_Sitestyle_Language_Direction + "</td><td>" + item._Fld_Sitestyle_Language_Text + "</td><td>" + item._Fld_Sitestyle_Language_Iso639_1 + "</td><td>" + item._Fld_Sitestyle_Language_Iso639_2 + "</td><td>" + item._Fld_Sitestyle_Language_Iso639_3 + "</td><td><a class='btn btn-primary' onclick='updateitem(" + item._Fld_Sitestyle_Language_Id + ")'>ویرایش</a><a class='btn btn-danger' onclick='deleteitem(" + item._Fld_Sitestyle_Language_Id + ")'>حذف</a></td>").appendTo('#table');
                    });
                },
                failure: function () {
                    alert("there is a error!");
        ...
        function updatesubmitdata() {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices.aspx/updateLanguage",
                contentType: "application/json",
                data: JSON.stringify({ obj: obj }),
                success: function (data) {
                    refreshdata();
