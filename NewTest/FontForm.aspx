<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FontForm.aspx.cs" Inherits="NewTest.FontForm" %>
...
<head>
    ...
    <script>
        ...
        function submitdata() {
            ...
            var obj =
            {
                "_Fld_Sitestyle_Font_Id": "0",
                "_Fld_Sitestyle_Font_Name": text,
                "_Fld_Sitestyle_Font_LanguageId": lang
            };

            $.ajax({
                type: "POST",
                url: "WebServices.aspx/createFont",
        ...
        function deleteitem(idd) {
            ...
            if (r == true) {
                ...
                $.ajax({
                    type: "POST",
                    url: "WebServices.aspx/deleteFont",
                    contentType: "application/json;charset=utf-8",
                    dataType: "json",
        ...
        function updateitem(idd) {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices.aspx/readFontById",
                contentType: "application/json;charset=utf-8",
                dataType: "json",
                data: JSON.stringify(data),
                success: function (response) {
                    $.each(response.d, function (i, item) {
                        $('#txtText').val(item._Fld_Sitestyle_Font_Name);
                        $('#DropdownLanguage').val(item._Fld_Sitestyle_Font_LanguageId);
                    });
                },
                failure: function () {
                    alert("there is a error!");
        ...
        function refreshdata() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    $.each(response.d, function (i, item) {
                        $('<tr>').html("<td>" + item._Fld_Sitestyle_Font_Name + "</td><td>" + item._Fld_Sitestyle_Font_Language + "</td><td><a class='btn btn-primary' onclick='updateitem(" + item._Fld_Sitestyle_Font_Id + ")'>ویرایش</a><a class='btn btn-danger' onclick='deleteitem(" + item._Fld_Sitestyle_Font_Id + ")'>حذف</a></td>").appendTo('#table');
                    });
                },
                failure: function () {
                    alert("there is a error!");
        ...
        function updatesubmitdata() {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices.aspx/updateFont",
                contentType: "application/json",
                data: JSON.stringify({ obj: obj }),
                success: function (data) {
                    refreshdata();
