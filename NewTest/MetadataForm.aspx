<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MetadataForm.aspx.cs" Inherits="NewTest.MetadataForm" %>
...
<head>
    ...
    <script>
        ...
        function submitdata() {
            ...
            var metadataobj =
            {
                "_Fld_Sitestyle_Metadata_Id": "0",
                "_Fld_Sitestyle_Metadata_Text": text,
                "_Fld_Sitestyle_Metadata_LanguageId": lang
            };

            $.ajax({
                type: "POST",
                url: "WebServices.aspx/createMetadata",
                contentType: "application/json",
                data: JSON.stringify({ obj: metadataobj }),
        ...
        function deleteitem(idd) {
            ...
            if (r == true) {
                ...
                $.ajax({
                    type: "POST",
                    url: "WebServices.aspx/deleteMetadata",
                    contentType: "application/json;charset=utf-8",
                    dataType: "json",
        ...
        function updateitem(idd) {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices.aspx/readMetadataById",
                contentType: "application/json;charset=utf-8",
                dataType: "json",
                data: JSON.stringify(data),
                success: function (response) {
                    $.each(response.d, function (i, item) {
                        $('#txtText').val(item._Fld_Sitestyle_Metadata_Text);
                        $("#DropdownLanguage").val(item._Fld_Sitestyle_Metadata_LanguageId);
                    });
                },
                failure: function () {
                    alert("there is a error!");
        ...
        function refreshdata() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    $.each(response.d, function (i, item) {
                        $('<tr>').html("<td>" + escapeHtml(item._Fld_Sitestyle_Metadata_Text ) + "</td><td>" + item._Fld_Sitestyle_Metadata_Language + "</td><td><a class='btn btn-primary' onclick='updateitem(" + item._Fld_Sitestyle_Metadata_Id + ")'>ویرایش</a><a class='btn btn-danger' onclick='deleteitem(" + item._Fld_Sitestyle_Metadata_Id + ")'>حذف</a></td>").appendTo('#table');
                    });
                },
                failure: function () {
                    alert("there is a error!");
        ...
        function updatesubmitdata() {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices.aspx/updateMetadata",
                contentType: "application/json",
                data: JSON.stringify({ obj: metadataobj }),
                success: function (data) {
                    refreshdata();
                ...
                <div id="Form" dir="rtl" class="col-lg-4 col-md-6 col-sm-12 col-12 text-center centered">
                    <form class="form-group" action="#" method="post" id="myform">
                        <div class="form-group">
                            <label class="bg-dark w-100 text-white">افزودن Metadata</label>
                        </div>
